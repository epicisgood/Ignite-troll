...
class Link_commands(commands.Cog):
  def __init__(self, client):
      self.client = client
  @bridge.bridge_command(aliases=["links"],description="Link dispenser for interstellar links")
  async def link(self, ctx):
    await ctx.reply(embed=discord.Embed(title="Links!", description="Click the link button below!", color=0x00ff00), view=Interstellar_buttons())
...
class Interstellar_buttons(discord.ui.View):
  ...
  async def link(self,button, interaction: discord.Interaction):
    if interaction.user.id == 726162926851063919: #Myself lol
      ...
      await interaction.response.send_message(content=f"{self.client.owner.mention} me for your personal link and ill add it to the link bot", ephemeral=True)


  @discord.ui.button(label="Public links", style=discord.ButtonStyle.secondary)
  async def public_links(self,button, interaction: discord.Interaction, ):
    links_public = {
